...
### 투약 누락(hold) 처리
- 각 요법 카드의 "투약 스케줄"에서 실제로 거른 회차를 클릭해 누락으로 표시
- 누락된 투약은 농도 예측·베이지안 피팅에서 제외되며, 이후 회차는 원래 시각 그대로 유지 (실제 hold 동작과 동일)
- 농도-시간 곡선 위에 누락 지점을 회색 X 마커로 표시
- 누락 후 시간표가 어긋나 재개된 경우 "+ 같은 용법으로 재개"로 용량·간격을 복제한 새 요법 생성
- 마지막 채혈 24시간 이내에 누락이 있으면 추정 신뢰도 저하 경고
- 누락을 입력하지 않으면 주지 않은 투약이 있었다고 가정하게 되어 청소율이 크게 과대추정됨 (내부 테스트 시나리오 기준 CL +75%)
...
3. **누락된 투약 표시 (해당 시)**
   요법 카드의 "투약 스케줄"을 펼쳐 거른 회차를 클릭 → 취소선으로 표시되고 계산에서 제외
   → 거른 뒤 원래 시간표가 아닌 다른 시각에 재개했다면 "+ 같은 용법으로 재개" 사용
...
│   ├── pk-calculations.js  # 약동학 계산 함수 (성인 + 소아 + 신생아, 다중 요법 중첩, 투약 누락)
